...
<script lang="ts">
  ...
  async function getItem (href: string): Promise<Attachment | undefined> {
    ...
    const match = href.match(/file\/(\S*)\/(\S*)\/(\S*)/)
    const objectId = match?.[2]
    if (objectId === undefined) return undefined
    return (
      await client.findAll(attachment.class.Attachment, { _id: objectId as Ref<Attachment> }, { limit: 1 })
    ).shift()
  }
      ...
      {#if item}
        ...
        >
          {#if isImage(item.mime)}
            <img src={getImageURL(item.url)} alt={item.name} />
          {:else}
            <AttachmentView {item} />
          {/if}
        </div>
