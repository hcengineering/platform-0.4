...
describe('elastic search', () => {
  it('should query model with params', async () => {
    ...
    const second = await model.findAll(core.class.Class, { _id: { $in: [txes[0].objectId as Ref<Class<Doc>>, txes[5].objectId as Ref<Class<Doc>>], type: '$in' } })
    expect(second.length).toBe(2)
    const third = await model.findAll(core.class.Space, { name: { $in: ['Sp1', 'Sp2'], type: '$in' } })
    expect(third.length).toBe(2)
    const like = await model.findAll(core.class.Space, { name: { $like: 'Sp*', type: '$like' } })
