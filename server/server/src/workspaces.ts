...
async function createWorkspace (workspaceId: string): Promise<Workspace> {
  ...
  const txStorage = await mongoConnection.createMongoTxStorage(workspaceId, hierarchy)
  const mongoDocStorage = await mongoConnection.createMongoDocStorage(workspaceId, hierarchy)

  // Build a hierarchy based on current set of transactions.
  const txes = (await mongoConnection.dumpDomain(workspaceId, DOMAIN_TX)) as Tx[]
